...
namespace CH
{
	...
	public:
		...
		void scale(const Vecotr3<Scalar>& scaling)
		{
	...
	Transform<Scalar>::inverseTimes(const Transform<Scalar>& t) const
	{
		...
		if (m_type & SCALING)
		{
			...
			return Transform<Scalar>(inv * t.getBasis(), inv * v, m_type | t.m_type);
		}
		else
		{
	...
	Transform<Scalar>::operator*(const Transform<Scalar>& t) const
	{
		return Transform<Scalar>(m_basis * t.m_basis, (*this)(t.m_origin), m_type | t.m_type);
	}
}
